...
let pathPlanning = new ROSLIB.Topic({
    ros: ros,
    name: '/move_plan',
    messageType : '/move_base/NavfnROS/plan' // 경로를 나타내는데 사용
});
...
function mapLoad() {
    ...
    const createPoseTopic = (oPerRatingMode) => {
        if (oPerRatingMode === 'slam') {
            ...
            console.log("geometry_msgs/PoseWithCovarianceStamped")
            return new ROSLIB.Topic({
                ros: ros,
                name: '/amcl_pose',
    ...
    pathPlanning.subscribe('/move_plan', function (message) {
        console.log("pathPlanning -> " + message.feedback.base_position.pose)
    ...
    const createFunc = function (handlerToCall, discriminator, robotMarker,oPerRatingMode) {
        console.log("createFunc 진입")
        console.log(discriminator.subscribe)
